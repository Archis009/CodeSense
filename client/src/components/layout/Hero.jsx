...
const Hero = () => {
  return (
    <section className="relative pt-32 pb-20 overflow-hidden">
      {/* Background Elements */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-[600px] bg-gradient-to-b from-primary/10 via-secondary/10 to-transparent blur-3xl -z-10" />
      ...
      <div className="container mx-auto px-6 text-center">
        ...
        >
          ...
          <h1 className="text-5xl md:text-7xl font-bold mb-8 leading-tight">
            Code Reviews on <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary">Autopilot</span>
          </h1>
          <p className="text-xl text-text-muted dark:text-slate-400 max-w-2xl mx-auto mb-10">
        ...
        >
          {[
            ...
            <div key={idx} className="p-6 rounded-2xl bg-surface/50 dark:bg-slate-800/50 backdrop-blur-sm border border-accent dark:border-slate-700">
              <feature.icon className="w-10 h-10 text-primary mb-4 mx-auto" />
              <h3 className="font-semibold text-lg mb-2">{feature.title}</h3>
              <p className="text-text-muted dark:text-slate-400">{feature.desc}</p>
            </div>
          ))}
        </motion.div>
      </div>
